...
public:
    ...
    void getMove(){
        ...
        {
            case 1:
                defaultEmptySymbol = 'X'; break;

            case 2:
                defaultEmptySymbol = '0'; break;
        }
        ...
        board[playerRow][playerColumn] = defaultEmptySymbol;
    ...
    void checkWin(){
        ...
        for (int i=0; i<sizeof(board[i])/sizeof(char); ++i) // Iterate through Y position
        {
            ...
            if (board[0][i] == PLAYER1_SYMBOL)
            {
                ...
                // Check for player2 symbol
            else if (board[0][i] == PLAYER2_SYMBOL)
            {
                // Iterate through X position
                for (int j=0; j<sizeof(board[j])/sizeof(char); ++j)
                    if (board[0][i] == board[j][i])
        ...
        if (board[0][2] != '.')
        {
            ...
            if (validationCounter == 3 && board[x][y] == PLAYER1_SYMBOL)
            {
                WIN = true;
                WINNER = 1;
            }
                // Check if 3 in a row AND last value validated was player 2 symbol
            else if (validationCounter == 3 && board[x][y] == PLAYER2_SYMBOL)
            {
                WIN = true;
                WINNER = 2;
            }
...
// TBD: return 4 corners of the board stored as an array?
//
//    int corners[4] = {board[0][0], board[3][0], board[0][3], board[3][3]};
//
//    for (int i=0; i<4; ++i)
//    {
//        // Validation Method: Diagonal.
//        if (corners[i] == PLAYER1_SYMBOL)
//    }
